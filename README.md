...
- **[Hybrid partial checkpoints](#hybrid-partial-checkpoints)**: Periodic snapshots of recurrent state during prefill, enabling fast restores on prompt mutations that SmartCache cannot cover. Achieves a 16-40x multi-turn speed-up with < 1% prompt processing (PP) overhead. [Benchmark](#benchmark).
...
All numbers on Qwen3.5-27B Q4_K_M, M4 Max 128 GB, Metal, `interval=256,512 slots=64`.
...
> Speedup 16-40x (mean 25x) on turn time, the longer the context and deeper the mutation, the greater the speedup.
...
- **Constant-size state:** Recurrent state is is cheap - O(1) with respect to token count.
  - The per-slot size depends solely on the model's SSM hidden dimensions and recurrent layer count, not on context length, parameter count, or quantisation.
  - For Qwen3.5-27B: `(n_embd_r=30720 + n_embd_s=786432) × 48 recurrent layers × 4 bytes (F32) = 149.6 MB/slot`.
...
Because per-slot size depends on the model's SSM dimensions rather than parameter count or quantisation, memory profiling is highly predictable.
...
- See the benchmark above to understand the trade-offs between interval, slots, and speedup.
